...
function CI(pull_request, branch, build_number, builder) {
...
function GetVersion(pull_request, branch, build_number)
{
	...
	else if(branch == null || !branch.match(/^(refs\/heads\/)?release[\/-]/i)) {
		release_version = '0.0';
	}
...
function GetPullRequest(pull_request){
	var pull_request = this.pull_request; 
	return !pull_request || pull_request.match(/false/i) ? null : pull_request;
}
