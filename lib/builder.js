...
Builder.prototype.Init = function() {
	if(!this.IsConfigured) {
		...
		if(!this.git_repository) {
			this.git_repository = origin_url.split('://')[1] || origin_url.split('@')[1]
		}

		if (this.git_repository == null) { throw 'git_repository is not specified'; }
		if (this.service_user == null) { throw 'service_user is not specified'; }

		//Set the service remote
		if(shell('git remote show service > /dev/null 2>&1 || echo 1'))
			shell("git remote add service https://" + this.service_user + '@' + this.git_repository	+ '	> /dev/null 2>&1 || exit 0');
...
Builder.prototype.PublishGitTag = function(pull_request, tag) {
	if(pull_request == null || !pull_request.match(/false/i)) { return; }
	if(tag == null) { throw 'tag not specified'; }
	this.Init();
	//Setup up deploy
	console.log(shell('git config --global user.email "builds@continuous-integration.io"'));
	console.log(shell('git config --global user.name "CI Builder"'));
	console.log(shell('git tag ' + tag + ' -a -m "Generated tag from CI."'));
	console.log('Pushing Git tag ' + tag);

	shell('git push --tags --quiet service ' + tag + ' > /dev/null 2>&1');
}

Builder.prototype.MergeDownstream = function(pull_request, branch, release_branch_name, master_branch_name) {
	if(pull_request == null || !pull_request.match(/false/i)) { return; }
	...
	console.log('Pushing merge commit to ' + branch + '.');
	try {
		shell('git push --quiet service HEAD:' + next_branch_to_merge + ' > /dev/null 2>&1');
	}
	catch (error) {
		console.log('Failed pushing to remote: ' + branch + '.  Possible issues include: protected branch, permissions issue, lack of token specified, connection failure');
		throw error;
	}
